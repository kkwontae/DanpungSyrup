...
class getUpdateTW(commands.Cog):
    ...
    @commands.command(aliases=['테섭패치'])
    async def LoadsInfo(self, message):
        ...
        embed.set_author(name='테스트서버 패치노트', icon_url='https://ssl.nx.com/s2/game/maplestory/renewal/common/test_world_icon_on.png')
...
def setup(client):
    client.add_cog(getUpdateTW(client))

def getUpdateLists(bs_obj):
    div = bs_obj.find("div", {"class" : "news_board"})
    ul = div.find("ul")
    lis = ul.findAll("li")
